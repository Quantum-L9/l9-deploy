...
module "firewall" {
 ...
 labels={environment="production",managed_by="l9-deployment-platform"}
}
module "server" {
 ...
 labels=merge({environment="production",managed_by="l9-deployment-platform"},{for role in each.value.roles : "role-${role}"=>"true"})
